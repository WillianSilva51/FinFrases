...
class TestCreateQuote:
    ...
    @pytest.mark.asyncio
    async def test_create_quote_failure(
        self, service, mock_repo, valid_request_data, quote
    ):
        expected_message = (
            f"A frase '{valid_request_data.content}' do autor "
            f"'{valid_request_data.author}' já existe"
        )

        mock_repo.get_by_content_and_author.return_value = quote
        ...
        mock_repo.get_by_content_and_author.assert_awaited_once_with(
